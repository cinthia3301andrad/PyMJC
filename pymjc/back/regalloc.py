...
class Liveness (InterferenceGraph):
    ...
    def build_interference_graph(self):
        '''
        We have a set of temporaries a, b, c,… that must be allocated 
        to registers r1,…, rk. A condition that prevents a and b from
        being allocated to the same register is called an interference.

        kinds of interference
        - When a and b are both live at the same program point, then they
        cannot be put in the same register.

        - when a must be generated by an instruction that cannot address register
        r1, then a and r1 interfere
